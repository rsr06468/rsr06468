(function(){
	...
	TT.functions = {
		...
		plugin_loaded: function(params){
			...
			if(params.extra_params) TT.setup.extra_params = params.extra_params;
	...
	TT.downloadInstaller = function(){
		...
		if(TT.functions.downloadStarted) {
			TT.functions.downloadStarted();
		}
		...
		for(var instance in TT.instances)
			...
			if(TT.functions.installing) {
				TT.functions.installing();
			}
			...
			var handler = function(callback){
				...
				img.onload = function(){
					...
					if(TT.functions.installComplete) {
						TT.functions.installComplete();
					}
					callback();
				};
